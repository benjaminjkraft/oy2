...
app.post('/api/users', async (c) => {
  ...
  if (!user) {
    try {
      const result = await c.env.DB.prepare('INSERT INTO users (username) VALUES (?)')
        .bind(username)
        .run();
...
app.get('/api/users/search', async (c) => {
  ...
  const users = await c.env.DB.prepare('SELECT id, username FROM users WHERE username LIKE ? LIMIT 20')
    .bind(`%${q}%`)
    .all();
...
app.post('/api/friends', async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare('INSERT OR IGNORE INTO friendships (user_id, friend_id) VALUES (?, ?)')
